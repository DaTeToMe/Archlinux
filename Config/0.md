...
sudo pacman -Syyu
```
# 文本编辑器 启动器 文件管理器及配件 文件管理器 浏览器 PDF查看器 视频播放器 网络管理器 音量调节（alsamixer，加M）网易云音乐
```
sudo pacman -S gedit rofi thunar gvfs-smb ranger google-chrome evince mplayer networkmanager alsa-utils netease-cloud-music
```
# 安装yaourt
```
